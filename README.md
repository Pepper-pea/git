...
在 Qt Creator 中运行后，先点击“启动”，等左侧摄像头画面出现。右侧“人员录入”区域填写姓名、班组、角色、证件号等信息，点击“录入当前人脸”，程序会随机要求眨眼、向左转头、向右转头或张嘴。动作通过后不要立刻离开画面，需要正对摄像头保持约 1 秒，程序会采集稳定正脸模板并保存到 `persons` 表。转头方向按使用者本人左右理解，不按屏幕画面左右理解。之后普通识别流程不要求眨眼，会直接按人脸匹配结果、名单类型和重复打卡规则决定是否开门，识别明细会继续写入 `recognized_faces` 表。
...
`config/device_config.json` 中的 `liveness.requireForRecognition` 默认是 `false`，表示普通门禁识别不强制眨眼。录入活体动作使用同一配置里的 `enrollmentChallengeTimeoutSeconds`、`mouthOpenRatio`、`turnOffsetRatio` 和 `stableFrames` 控制。录入动作通过后会额外采集正脸模板；如果旧人员曾在转头或张嘴帧被保存导致识别不到，请删除或覆盖重新录入一次。
