...
bool FaceRecognizer::loadModels() { // 实现模型加载函数。
    ...
    if(useDlib_) { // 判断是否要求使用 dlib。
        if(shapePredictorPath_.isEmpty() || !QFileInfo::exists(shapePredictorPath_)) { // 判断 dlib 68 点模型文件是否存在。
            emit logMessage(QStringLiteral("WARN"), QStringLiteral("dlib 68 点模型不存在：%1，将使用 Haar 基础录入/识别").arg(shapePredictorPath_)); // 发出降级日志。
        } else { // 处理模型文件存在。
            try { // 捕获 dlib 反序列化异常。
                ...
                emit logMessage(QStringLiteral("WARN"), QStringLiteral("dlib 68 点模型加载失败：%1，将使用 Haar 基础录入/识别").arg(QString::fromLocal8Bit(error.what()))); // 发出降级日志。
            } // 结束异常处理。
        } // 结束文件存在判断。
    } // 结束 dlib 启用判断。
#else // 处理未启用 dlib 的构建。
    if(useDlib_) { // 判断配置是否要求 dlib。
        emit logMessage(QStringLiteral("WARN"), QStringLiteral("当前构建未启用 dlib，将使用 Haar 基础录入/识别，未启用 68 点活体")); // 发出降级日志。
    } // 结束 dlib 配置判断。
#endif // 结束 dlib 判断。
...
RecognitionResult FaceRecognizer::recognize(const QImage& image) { // 实现识别函数。
    ...
    if(livenessAvailable() && result.detection.landmarks.size() < 68) { // 判断是否缺少真实 dlib 68 点关键点。
        result.status = RecognitionStatus::LowQuality; // 标记为识别条件不足。
        result.message = QStringLiteral("dlib 68 点关键点未就绪"); // 记录提示文本。
        lastError_ = result.message; // 保存失败原因。
        return result; // 返回结果。
    } // 结束 dlib 关键点判断。
    ...
    if(livenessAvailable()) { // 判断是否启用 68 点活体检测。
