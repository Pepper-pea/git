...
bool FaceRecognizer::loadModels() { // 实现模型加载函数。
    ...
    if(useDlib_) { // 判断是否要求使用 dlib。
        if(shapePredictorPath_.isEmpty() || !QFileInfo::exists(shapePredictorPath_)) { // 判断 dlib 68 点模型文件是否存在。
            emit logMessage(QStringLiteral("ERROR"), QStringLiteral("dlib 68 点模型不存在：%1").arg(shapePredictorPath_)); // 发出错误日志。
        } else { // 处理模型文件存在。
            try { // 捕获 dlib 反序列化异常。
                ...
                emit logMessage(QStringLiteral("ERROR"), QStringLiteral("dlib 68 点模型加载失败：%1").arg(QString::fromLocal8Bit(error.what()))); // 发出错误日志。
            } // 结束异常处理。
        } // 结束文件存在判断。
    } // 结束 dlib 启用判断。
#else // 处理未启用 dlib 的构建。
    if(useDlib_) { // 判断配置是否要求 dlib。
        emit logMessage(QStringLiteral("ERROR"), QStringLiteral("当前构建未启用 dlib，无法使用 68 点关键点模型")); // 发出错误日志。
    } // 结束 dlib 配置判断。
#endif // 结束 dlib 判断。
...
RecognitionResult FaceRecognizer::recognize(const QImage& image) { // 实现识别函数。
    ...
    if(useDlib_ && result.detection.landmarks.size() < 68) { // 判断是否缺少真实 dlib 68 点关键点。
...
bool FaceRecognizer::enrollPerson(Person person, const QImage& image) { // 实现人员特征录入函数。
    ...
    if(useDlib_ && faces.first().landmarks.size() < 68) { // 判断是否缺少真实 dlib 68 点关键点。
