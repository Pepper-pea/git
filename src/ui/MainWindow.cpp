...
void MainWindow::processEnrollmentChallenge(const QImage& image, qint64 frameId) { // 实现录入活体动作挑战处理函数。
    ...
    if(elapsedSeconds > enrollmentChallengeTimeoutSeconds_) { // 判断挑战是否超时。
        ...
        const QString diagnostic = QStringLiteral("录入动作超时诊断：动作=%1，人脸=%2，68点=%3，最后转头偏移=%4，阈值=%5").arg(actionText, enrollmentLastFaceDetected_ ? QStringLiteral("已检测到") : QStringLiteral("未检测到"), enrollmentLastLandmarksReady_ ? QStringLiteral("就绪") : QStringLiteral("未就绪"), QString::number(enrollmentLastHeadTurnOffset_, 'f', 3), QString::number(enrollmentTurnOffsetThreshold_, 'f', 3)); // 生成诊断日志。
