...
QString enrollmentChallengeText(EnrollmentChallengeType challenge) { // 将录入活体动作转换成界面提示文本。
    ...
    case EnrollmentChallengeType::TurnLeft: return QStringLiteral("向画面左侧转头"); // 左侧转头动作。
    case EnrollmentChallengeType::TurnRight: return QStringLiteral("向画面右侧转头"); // 右侧转头动作。
...
EnrollmentChallengeType randomEnrollmentChallenge() { // 随机生成一个录入活体动作。
    ...
    case 1: return EnrollmentChallengeType::TurnLeft; // 选择向画面左侧转头。
    case 2: return EnrollmentChallengeType::TurnRight; // 选择向画面右侧转头。
...
void MainWindow::loadConfig() { // 实现配置读取函数。
    ...
    enrollmentLiveness_.setTurnOffsetThreshold(liveness.value(QStringLiteral("turnOffsetRatio")).toDouble(0.30)); // 设置录入转头阈值。
    enrollmentLiveness_.setStableFrameCount(liveness.value(QStringLiteral("stableFrames")).toInt(3)); // 设置动作连续通过帧数。
    enrollmentChallengeTimeoutSeconds_ = qMax(1, liveness.value(QStringLiteral("enrollmentChallengeTimeoutSeconds")).toInt(10)); // 设置录入动作超时时间。
...
void MainWindow::processEnrollmentChallenge(const QImage& image, qint64 frameId) { // 实现录入活体动作挑战处理函数。
    ...
    if(elapsedSeconds > enrollmentChallengeTimeoutSeconds_) { // 判断挑战是否超时。
        ...
        const QString reason = QStringLiteral("录入失败：动作超时，请重新点击录入并完成%1").arg(actionText); // 生成失败原因。
    ...
    if(faces.isEmpty()) { // 判断是否没有检测到人脸。
        enrollStatusLabel_->setText(QStringLiteral("正在检测动作：%1，请让人脸位于画面中央（剩余 %2 秒）").arg(actionText, QString::number(remainingSeconds))); // 提示调整位置。
        return; // 等待下一帧。
    } // 结束人脸判断。
    FaceDetection detection = faces.first(); // 取最大人脸作为录入目标。
    if(detection.landmarks.size() < 68) { // 判断是否缺少关键点。
        enrollStatusLabel_->setText(QStringLiteral("正在等待 dlib 68 点关键点，请正对摄像头（剩余 %1 秒）").arg(remainingSeconds)); // 提示关键点状态。
        return; // 等待下一帧。
    } // 结束关键点判断。
    if(!enrollmentLiveness_.updateChallenge(detection, enrollmentChallenge_)) { // 判断当前动作是否尚未通过。
        enrollStatusLabel_->setText(QStringLiteral("正在检测动作：%1，通过后自动保存（剩余 %2 秒）").arg(actionText, QString::number(remainingSeconds))); // 更新动作提示。
        return; // 等待下一帧。
    } // 结束动作判断。
    const Person enrolledPerson = pendingEnrollmentPerson_; // 保存待录人员副本。
    const bool saved = recognizer_.enrollPerson(enrolledPerson, image); // 动作通过后自动保存当前人脸。
    ...
    appendLogRow(QStringLiteral("INFO"), QStringLiteral("录入人员：%1（%2），动作：%3，帧号 %4").arg(enrolledPerson.name, enrolledPerson.id, actionText, QString::number(frameId))); // 写入录入日志。
...
void MainWindow::enrollCurrentFace() { // 实现当前画面人员录入函数。
    ...
    enrollStatusLabel_->setText(QStringLiteral("请在 %1 秒内完成：%2。通过后会自动保存。").arg(QString::number(enrollmentChallengeTimeoutSeconds_), actionText)); // 更新挑战提示。
...
void MainWindow::clearEnrollmentForm() { // 实现录入表单清空函数。
    if(enrollmentChallengeActive_) { // 判断是否正在进行录入活体动作。
        ...
        appendLogRow(QStringLiteral("INFO"), QStringLiteral("录入活体动作已取消")); // 写入取消日志。
    } // 结束活体动作取消。
