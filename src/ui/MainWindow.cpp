...
QJsonObject readConfigFile(const QString& path, QString* projectRoot) { // 定义配置文件读取函数。
    ...
    for(const QString& candidate : candidates) { // 遍历候选路径。
        ...
        if(document.isObject()) { // 判断是否为对象。
            if(projectRoot) { // 判断是否需要回传项目根目录。
                QDir configDir = QFileInfo(candidate).absoluteDir(); // 取得配置文件所在目录。
                if(configDir.dirName() == QStringLiteral("config")) { // 判断是否位于 config 目录。
                    configDir.cdUp(); // 回到项目根目录。
                } // 结束目录判断。
                *projectRoot = configDir.absolutePath(); // 保存项目根目录。
            } // 结束项目根目录回传。
...
void MainWindow::loadConfig() { // 实现配置读取函数。
    QString projectRoot = QDir::currentPath(); // 准备用于解析本地数据目录的项目根目录。
...
void MainWindow::enrollCurrentFace() { // 实现当前画面人员录入函数。
    ...
    if(!saved) { // 判断录入是否失败。
        const QString reason = DatabaseManager::instance().lastError().isEmpty() ? QStringLiteral("请确认画面中有人脸，并重新录入") : DatabaseManager::instance().lastError(); // 生成失败原因。
